# Hello, moi c'est Mathéo 👋

🎓 Computer Science student at the University of Rennes  
🤖 Full-Stack AI Engineer  

## Skills & Interests

- Typescript, Java, Python
- Langchain like libraries (RAG, agents, MCP...)
- CI/CD, Docker, Pipelines

## Most Notable Projects

### [aaron](https://gitlab.com/aaronngbot) - ETL App to provide good plans to users, with a Discord bot interface.

[🌐 Website](https://aaronlem.ovh)
📦 Microservices, load balancing and sharding
🚄 MongoDb & Redis Queues
🎨 UI/UX and plan core features

### Hackaton - Developped an sport app in 24h, with a team of 4 people.
backend and frontend
[📱 Frontend](https://github.com/Bigchef-dev/hackaton-front) - developped by my pairs
[⚡ Backend](https://github.com/Bigchef-dev/hackaton-back) - developped by me, for the needs of my pairs.

### Telegram MCP Client & Server - An Personal assistant, with a Telegram interface, to add custom MCP skills and agents.
[🤖 Telegram MCP Client](https://github.com/Bigchef-dev/telegram-mcp-client) : 
With Mastra and Mistral models, supports voice and multimodal inputs
[⚡ Telegram MCP Server](https://github.com/Bigchef-dev/telegram-mcp-server) :

## Contact

[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Bigchef-dev)
[![GitLab](https://img.shields.io/badge/GitLab-FC6D26?style=for-the-badge&logo=gitlab&logoColor=white)](https://gitlab.com/BigChef_)